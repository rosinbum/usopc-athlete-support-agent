...
export const AgentStateAnnotation = Annotation.Root({
  ...
  /**
   * Domain-aware emotional support context generated by the emotionalSupport
   * node. When present, the synthesizer uses this instead of generic empathy
   * preambles for more situation-specific, trauma-informed responses.
   */
  emotionalSupportContext: Annotation<EmotionalSupportContext | undefined>({
    reducer: (_prev, next) => next,
    default: () => undefined,
  }),
  ...
  /**
   * Whether the retrieval expander has already attempted query reformulation.
   * Prevents infinite loops between retriever and expander.
   */
  expansionAttempted: Annotation<boolean>({
    reducer: (_prev, next) => next,
    default: () => false,
  }),

  /**
   * Reformulated queries generated by the retrieval expander node.
   * Used for tracing and debugging expansion attempts.
   */
