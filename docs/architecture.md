...
## Infrastructure (SST + EC2)

Defined in `sst.config.ts`. Deployed stages (staging, production) use a hybrid approach: the **Next.js web app and Slack bot run on an EC2 instance** (t3.small) with PM2 + Nginx, while **Lambda workers, crons, and all AWS-managed resources** are provisioned by SST. See [AWS Container Strategy](./aws-container-strategy.md) for the full rationale.

Dev stages use local Docker Postgres and SST Lambda emulation.

| Component | Development                    | Production                                   |
| --------- | ------------------------------ | -------------------------------------------- |
| Database  | Local Docker Postgres          | Neon Postgres (via SST Secret)               |
| Slack API | Local Lambda emulation via SST | EC2 — Hono + Node.js server (PM2, :3001)     |
| Web       | Next.js dev server             | EC2 — Next.js standalone server (PM2, :3000) |
| Workers   | Local Lambda emulation via SST | Lambda + SQS                                 |
| Crons     | Manual triggers                | Lambda + EventBridge                         |
| Secrets   | SST dev secrets                | SST secrets (via `sst shell` on EC2)         |

**SST Resources (all stages):**
...
- **SQS**: `DiscoveryFeedQueue`, `IngestionQueue` (with DLQs)
- **Crons**: `DiscoveryCron` (weekly discovery), `IngestionCron` (weekly ingestion), `CheckpointCleanupCron` (daily)

**EC2 Instance (production/staging):**

- **PM2** manages both processes (`ecosystem.config.cjs`)
- **Nginx** reverse proxy handles TLS termination and routing
- Secrets injected via `sst shell` or `.env.ec2` (generated by `scripts/sync-sst-env.sh`)
- Deploy via `scripts/deploy-ec2.sh` (git pull + build + pm2 restart)
